...
//        identifierNumber = String.valueOf(generateFakeNumber());
        ...
        Assert.assertEquals(polictyPage.verifyReleasePolicySuccess(), "Đang có hiệu lực");
    ...
    public void User_01_Create_Policy(Method method) throws Exception {
        ...
        sdbsPage.selectDropdownKindOfSDBS(driver, "Sửa đổi bổ sung cho khách hàng");

        sdbsPage.selectDropdownTypeOfSDBS(driver, "Tăng người được bảo hiểm");

        sdbsPage.clickToSubmitCreateButton();

        Assert.assertEquals(sdbsPage.verifyCreateSDBS(), "Tạo mới sửa đổi bổ sung");

        sdbsPage.clickToConfirmCreateSDBS();

        Assert.assertEquals(sdbsPage.verifyMessageSDBS(), "Không có thay đổi số lượng người ở danh sách người được bảo hiểm.");
